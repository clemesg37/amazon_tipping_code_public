...
| `cluster/species_richness_from_SDM_projections/` | `calculate_richness_cluster_final.R` and `calculate_mean_richness_cluster.R` | Creates historical, future, and delta species-richness rasters from SDM projections for each MC sample, then calculates their MC means. |
...
The richness scripts first create one raster per taxonomic input. `combine_global_SR.R` then creates `SR_world_all_taxa_land_05deg.tif` by summing the five required rasters. Copy this compact combined raster to the Figure 1 local data bundle after the cluster run.

## Inputs and outputs

| Input | Location | Used by | Output |
| --- | --- | --- | --- |
| Per-species SDM area summaries | `$AMAZON_DATA_DIR/intermediate/06_sdm/models/<species>/area_results/area_all_MC_models.csv` | `create_full_dataset_area_change.py` | `$AMAZON_DATA_DIR/intermediate/07_postprocess_data/sdm_area_change/final_amazon_area_ETresid_updateds.csv` |
| SDM projection rasters, MC design, species metadata, and Amazon mask | `$AMAZON_DATA_DIR/intermediate/06_sdm/models/`, `06_sdm/mc_samples/MC_sample.csv`, `04_rasterized_species/full_species_list_amazon_updated.csv`, and `01_c_noresm2/amazon_mask/amazon_mask.tif` | SDM-projection SR scripts | `species_richness_from_SDM_projections/results_ETresid/`, `results_mean_ETresid/`, `results_by_species_taxa/`, and `results_by_class/` |
