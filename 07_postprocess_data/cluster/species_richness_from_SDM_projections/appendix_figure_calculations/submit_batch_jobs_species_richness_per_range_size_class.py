...
# Edit this list to submit only range-size classes required for the current run.
SELECTED_CLASSES = ["supersmall", "small", "medium", "large", "superlarge"]
...
for target_class in SELECTED_CLASSES:
    for part in np.array_split(np.arange(1, 101), N_MC_SAMPLE_SPLITS):
        ...
        job_name = f"SR_{target_class}_{start_index}_{end_index}"
        command = f"Rscript --vanilla {SCRIPT_DIR / 'calculate_SR_by_range_class.R'} {target_class} {start_index} {end_index}"
