"""Submit path-neutral all-species mean and SD richness jobs."""
...
def validate_total_outputs(directory: Path) -> None:
    missing = [
        directory / f"{kind}_species_richness_MC_all_species_{mc_id}.tif"
        for kind in MAP_KINDS
        for mc_id in range(1, 101)
        if not (directory / f"{kind}_species_richness_MC_all_species_{mc_id}.tif").is_file()
    ]
    if missing:
        examples = "\n".join(str(path) for path in missing[:20])
        raise RuntimeError(
            f"Cannot calculate mean/SD: {len(missing)} all-species MC map(s) "
...
def make_batch_script(job_name: str, arguments: list[str]) -> Path:
    ...
    command = " ".join(
        [
            "Rscript", "--vanilla", shlex.quote(str(SCRIPT_DIR / "calculate_mean_richness_cluster.R")),
    ...
    for tip in TIPS:
        ...
        for deforestation in deforestation_values:
            for period in TIME_PERIODS:
                ...
                validate_total_outputs(directory)
                job_name = f"SRmean_{ssp}_{tip}_{deforestation}_{period}"
