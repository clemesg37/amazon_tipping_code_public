...
if (length(args) != 7) {
  stop(
    "Usage: calculate_richness_cluster_final.R ",
    "<start_index> <end_index> <ssp> <tip> <deforestation> <period> <species_type>"
  )
}
...
bird_seasonality <- dplyr::case_when(
...
if (taxon_type == "birds") {
  species_df <- species_df |>
    dplyr::filter(type == taxon_type, seasonality == bird_seasonality)
} else {
  species_df <- species_df |> dplyr::filter(type == taxon_type)
}

all_species <- species_df$Species
if (length(all_species) == 0) {
  stop("No species selected for species_type = ", species_type)
}
message("Selected ", length(all_species), " species for ", species_type)

mc_samples <- read.csv(file.path(
...
amazon_rast <- terra::rast(file.path(
...
for (mc_i in start_index:end_index) {
  message("Processing ", species_type, ", MC sample ", mc_i)

  ms <- mc_samples |> dplyr::filter(mc_id == mc_i)
  algo <- ms$algo
  run <- ms$model_run
  prec <- ms$prec_sample
  ...
  for (species in all_species) {
    result <- tryCatch({
      ...
      future <- if (tip == "notip") {
        ...
        load_future_tss_tip(species, algo, run, prec)
      }

      list(
        historic = mask_to_amazon(historic),
        future = mask_to_amazon(future)
      )
  ...
  if (length(failures) > 0) {
    stop(
      "No output was written for ", species_type, ", MC ", mc_i,
  ...
  terra::writeRaster(
    historic_richness,
    file.path(output_dir, paste0("historic_species_richness_MC_", species_type, mc_i, ".tif")),
    overwrite = TRUE
  )
  terra::writeRaster(
    future_richness,
    file.path(output_dir, paste0("future_species_richness_MC_", species_type, mc_i, ".tif")),
    overwrite = TRUE
  )
  terra::writeRaster(
    delta_richness,
    file.path(output_dir, paste0("delta_species_richness_MC_", species_type, mc_i, ".tif")),
    overwrite = TRUE
  )
}